...
public class JWTService {
	...
	private Key getSignKey() {
		return Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8)); //Keys.hmacShaKeyFor->crete key which used to sign and verify jwt
	}
//suppose user login with abc@gmail.com after login succeed 
	...
	public String generateToken(String email) {
		return Jwts.builder() //start creating jwt
				...
				.setExpiration(new Date(System.currentTimeMillis() + expiration)) //currenttime=5am, exptime=1hr it store 6am
				.signWith(getSignKey(), SignatureAlgorithm.HS256).compact(); //it create something like eyJhbGciOiJIUzI1NiJ9.... which is JWt
...
//it will contain abc@gmail.com-----Issue time---Expiry time all signed with secret key
	...
	public String extractEmail(String token) { //from frontend token will like Bearer eyJhbGc.... filter will remove bearer and send to extractEmail 
	    Claims claims = Jwts.parserBuilder() //start reading jwt 
                .setSigningKey(getSignKey()) //Use secret key if wrong exception occure
                .build()
                .parseClaimsJws(token) //Verify Signature,Expiry and Token format
	...
	public boolean isTokenValid(String token) { //Check weather JWT has expired
		...
		Date expiry=Jwts.parserBuilder().setSigningKey(getSignKey()).build().parseClaimsJws(token).getBody().getExpiration();
